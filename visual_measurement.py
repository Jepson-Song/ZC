...
def udp_server():
    # global vec
    # open udp server
    udp_server = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # ipv4,udp
    udp_server.bind(('127.0.0.1', 54377))  # UDP服务器端口和IP绑定

    while (True):
        print('正在等待接收客户端信息...')
        buf, addr = udp_server.recvfrom(40960)
        msg = buf.split()
        data = []
        for i in msg:
            data.append(float(i.decode('utf-8')))
        global vec
        vec = np.array(data)
        #print(vec)
...
if __name__ == '__main__':
    ...
    url = "http://192.168.31.187:8080/?action=stream"
    open_time = time.time()
    ...
    # udp server 线程
    threading.Thread(target=udp_server, name="One").start()
...
#        image_points = np.vstack((landmarks[30],landmarks[8],landmarks[36],landmarks[45],landmarks[48],landmarks[54]))
        ...
        print(vec)
        (nose_end_point2D, jacobian) = cv2.projectPoints(np.array([(0.0, 0.0, 1000),vec*1000]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
        print(nose_end_point2D)
        print(nose_end_point2D.shape)
        for i in range(8):
        ...
        p3 = ( int(nose_end_point2D[1][0][0]), int(nose_end_point2D[1][0][1]))
